...
def verify_indexing():
    ...
    try:
        ...
        facet_resp = requests.get("http://localhost:8983/solr/dummy_data/select?q=*:*&rows=0&facet=true&facet.field=category&wt=json")
        ...
        price_resp = requests.get("http://localhost:8983/solr/dummy_data/select?q=price:[300%20TO%20*]&rows=0&wt=json")
        price_resp.raise_for_status()
        print(f"  Count: {price_resp.json()['response']['numFound']}")

        # 4. In Stock Items
        print("\n--- In Stock Items ---")
        stock_resp = requests.get("http://localhost:8983/solr/dummy_data/select?q=in_stock:true&rows=0&wt=json")
